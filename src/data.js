...
// kept for any legacy callers — new UI uses MEETING_TYPES below
...
// Maps the new MEETING_TYPES/ENVIRONMENTS_V2 vocabulary onto the internal
// purpose/environment buckets that generateComprehensiveAnalysis already understands.
const MEETING_TYPE_TO_PURPOSE = {
  first_date: 'romantic',
...
const ENV_V2_TO_LEGACY = {
  cafe: 'quiet_cafe',
  restaurant: 'loud_restaurant',
  outdoor: 'outdoor',
  travel: 'outdoor',
  event: 'loud_restaurant',
}

// Oracle Report: reshapes the analysis into the four-chapter ritual structure
// (First Impression / Hidden Frequency / Things To Avoid / Perfect Conversation Topics).
export function generateOracleReport({ zodiac, mbti, meetingType, environment, energy, screenshotScores }) {
  const purpose = MEETING_TYPE_TO_PURPOSE[meetingType] ?? 'friend'
  const legacyEnv = ENV_V2_TO_LEGACY[environment] ?? 'quiet_cafe'
  const base = generateComprehensiveAnalysis({
    zodiac,
    mbti,
    purpose,
  ...
  return {
    ...
    godName: base.godName,
    compatibility: base.survivalRate,
    firstImpression: base.macroAssessment,
    hiddenFrequency,
    thingsToAvoid: base.forbidden,
    rescueLine: base.rescueLine,
  }
}
