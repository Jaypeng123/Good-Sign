...
export default function App() {
  ...
  async function executeAnalysis() {
    ...
    // 優先使用 Vercel 環境變數，次之使用你提供的金鑰作為 Fallback 確保即時可用
    ...
    try {
      ...
      console.warn("AI 觀測失敗，啟用在地防崩潰備用大腦：", err);
      // 降級 Fallback 處理
  ...
  async function handleSendMessage() {
    ...
    try {
      ...
      setChatMessages(prev => [...prev, { sender: 'guardian', text: `（星軌微鳴... 守護者感受到了你的誠意，對你的話報以溫和的注視。）` }]);
    } finally {
      setIsTyping(false);
    }
  }
              ...
              <div className="glass-panel p-8 border border-[#2C2A29]/10">
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2" onMouseLeave={() => setHoveredZodiacId(null)}>
                      {ZODIACS.map((z) => {
                        ...
                        return (
                          <button
                            ...
                            style={{ filter: isDimmed ? 'blur(2px)' : 'none', opacity: isDimmed ? 0.4 : 1 }}
                            className={`p-3 border text-xs font-serif transition-all duration-300 ${
                              isSelected ? 'bg-[#2C2A29] text-[#F6F3ED] border-[#2C2A29]' : 'bg-white/20 border-[#2C2A29]/10 hover:border-[#2C2A29]/40'
                            }`}
                          >
                            {z.label}
                          </button>
